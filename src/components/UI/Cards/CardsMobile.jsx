...
import classes from './CardsMobile.module.css'
...
const CardsMobile = ({id, rate, type, page, moviedata}) => {
            ...
            <Link style={{ textDecoration: 'none' }} className={classes.card_container} to={`/${type}/${movie.id}`} key={movie.id}>
                ...
                <div className={classes.text_container}>
                    ...
                    <div className={classes.description_container}>
                        <p className={classes.year}>{type == 'tv' ? movie ? movie.first_air_date.slice(0,4) : "" : movie ? movie.release_date.slice(0,4) : ""}</p>
                        <div className={classes.point}></div>
                        <p className={classes.rating}>
                            {rate ? rate : movie ? movie.vote_average.toString().slice(0, 3) : ""}
                            <svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24" className={`icon ${classes.icon}`}><path d="m233-80 65-281L80-550l288-25 112-265 112 265 288 25-218 189 65 281-247-149L233-80Z"/></svg>
                        </p>
                    </div>
                </div>
                {/* {onSeeLater ?
